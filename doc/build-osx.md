...
## Dependencies
```shell
brew install automake berkeley-db62 libtool boost miniupnpc openssl pkg-config python qt libevent qrencode
```
...
1. Clone the Blackcoin More source code:
    ```shell
...
The first time you run blackmored, it will start downloading the blockchain. This process could
...
./src/blackmored-cli --help    # Outputs a list of command-line options.
./src/blackmored-cli help      # Outputs a list of RPC commands when the daemon is running.
```
