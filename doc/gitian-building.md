...
*Setup instructions for a gitian build of Bitcloud Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the Bitcloud
...
to the Bitcloud GitHub release page.
...
- [Building Bitcloud Core](#building-bitcloud-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/bitcloud-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/bitcloud-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/bitcloud-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/bitcloud-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/bitcloud-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/bitcloud-win-${VERSION%\.*}-build.assert
```
...
```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/bitcloud-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/bitcloud-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/bitcloud-osx-*-build.assert
```
