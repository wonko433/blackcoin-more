...
BOOST_FIXTURE_TEST_CASE(checkinputs_test, TestChain100Setup)
{
...

    {
        ...
        CValidationState state;
        PrecomputedTransactionData txdata(tx);
        // This transaction is now invalid under segwit, because the second signature is missing.
        BOOST_CHECK(!CheckInputs(CTransaction(tx), state, pcoinsTip.get(), SCRIPT_VERIFY_P2SH, true, true, txdata, nullptr));

        std::vector<CScriptCheck> scriptchecks;
        // Make sure this transaction was not cached (ie because the first
        // input was valid)
        BOOST_CHECK(CheckInputs(CTransaction(tx), state, pcoinsTip.get(), SCRIPT_VERIFY_P2SH, true, true, txdata, &scriptchecks));
