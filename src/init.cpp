...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-blockmaxsize=<n>", strprintf(_("Set maximum block size (default: %d)"), DEFAULT_BLOCK_MAX_SIZE));
...
void InitParameterInteraction()
{
    ...
    if (gArgs.IsArgSet("-blockmaxsize")) {
        ...
        if (gArgs.SoftSetArg("blockmaxsize", strprintf("%d", max_size))) {
            LogPrintf("%s: parameter interaction: -blockmaxsize=%d -> setting -blockmaxsize=%d\n", __func__, max_size, max_size);
        } else {
            LogPrintf("%s: Ignoring blockmaxsize setting which is overridden by blockmaxsize", __func__);
        }
    }
}
