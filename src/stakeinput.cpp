...
// The zBTDX block index is the first appearance of the accumulator checksum that was used in the spend
...
bool CZPivStake::GetModifier(uint64_t& nStakeModifier)
{
    ...
    // zBTDX staking is disabled long before block v7 (and checkpoint is not included in blocks since v7)
...
{
    //The unique identifier for a zBTDX is a hash of the serial
...
{
    //Create an output returning the zBTDX that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZPIVOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zBTDX output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zBTDX", __func__);
